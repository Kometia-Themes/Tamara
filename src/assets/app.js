...
common: {
  init: function() {
    ...
    //** Añadir clase de embed de BS a contenedores de video
    ...
    var showAjaxCart = function() {
      ...
      setTimeout(function(){ hideAjaxCart(false); }, 2600);
    }
    ...
    var buildAjaxCart = function(obj, sku, data, total) {
      clearAjaxCart();
        ...
        $.each(data, function(_, value) {
            if (sku === value.sku.id) {
                var image_url = value.sku.image_url ? value.sku.image_url + '&w=100&fit=crop' : "{{ 'placeholders/product-11.jpg' | global_img_url }}";
    ...
    var addSimpleProductToCarByAjax = function(sku, qty, linkId) {
      var linkId = (linkId == undefined || linkId == null) ? false : true;
      var productSku = sku;
      var qty = qty == null ? 1 : qty;
      var wasSuccessful = function(data) {
        if(linkId) {
          $('a').data('skuId') == sku ///////
          console.log(data);
        }
      }
      // WIP pass link id to reenable it
      $.ajax({
        type: "POST",
        url: ajaxConfig.postUrl,
        data: {'sku_id':productSku,'quantity':qty},
    ...
    $('.ajaxlink').on('click', function(e){
      if(!$(this).hasClass('disabled')) {
        ...
        addSimpleProductToCarByAjax(sku, 1, sku);
      }
    });
...
products: {
  init: function() {
    //** Localstorage for save display mode
    var listMode = function() {
      return localStorage.listMode ? localStorage.listMode : localStorage.listMode = 'grid';
    }

    //** Poner a los productos la altura del más alto para no romper el layout
    var maxHeight = 0,
        thumbContFw = 'thumb-cont__full-width',
...
collections: {
  init: function() {
    var mq = matchMedia('only screen and (min-width: 768px)');
    if (!mq.matches) {
      $('.thumbnail-container').each(function(){
        $(this).find('.thumbnail-img').height($(this).find('.thumbnail-img').width());
      });
    }
    $('.thumbnail-img').addClass('imgLiquid').imgLiquid();
  }
},
category: {
  init: function() {
...
blog: {
  init: function() {
    var thumbContFw = 'thumb-cont__full-width',
        thumbCont = $('.thumbnail-container:not(.'+ thumbContFw +')'),
        mq = matchMedia('only screen and (max-width: 767px)'),
        i = 0;
  }
...
has_featured: {
  init: function() {
    // Quitar margin del nav-wrapper para que no empuje el jumbotron
    $('.nav-wrapper').css('margin-bottom', '0');
  }
},
has_sidebar: {
  init: function() {
